...
export async function _enableModule(
	safeAddr: string,
	safeSDK: any,
	coordinates: any,
	credentialId: any
	...
	const enableModuletx = await safeSDK.createEnableModuleTx(
		addresses.safeProotcolManager
	);

	const pluginIface = new ethers.utils.Interface(RecoveryPlugin.abi);
	const addWebAuthnRecovTx = pluginIface.encodeFunctionData(
		"addWebAuthnRecover",
		[
			5184000, // 60 days
			coordinates,
	...
	const paddedAddress = ethers.utils.hexZeroPad(
		`0x${safeAddr.slice(2, 32).toString()}`,
		32
	);
	...
	const txResponse1 = await safeSDK.executeTransaction(safeTransaction1, {
		gasLimit: 300000,
	});
